...
begin
  ...
  begin
    ...
    AddToInitProc(swalSuccess('Confirmação', ' Incluído  com sucesso !!!'));
  ...
  begin
    ...
    AddToInitProc(swalSuccess('Confirmação', ' Atualizado  com sucesso !!!'));
...
begin
  ...
  if EDITPARAM.Text = 'A' then // alteração de dados
  begin
...
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          WebApplication.CallBackResponse.AddJavaScriptToExecute
            (swalConfirm('CONFIRMAÇÃO',
            'Salvo com sucesso, deseja Aprovar Gerência agora ?', 'warning',
            'Sim', 'Não', 'BTNAPRGER'));
        end
        else
        begin
          WebApplication.CallBackResponse.AddJavaScriptToExecute
            (swalConfirm('CONFIRMAÇÃO',
            'Salvo com sucesso, deseja fechar o formulário ?', 'warning', 'Sim',
            'Não', 'BTNSAIR'));
        end;
  ...
  begin
    ...
    AddToInitProc(swalAlert('Alerta',
      'Existem Documentos Obrigatórios não informados,' + docs_obg +
      ' verifique!!!'));
  end;
...
begin
  ...
  begin
    ...
    begin
      AddToInitProc(swalAlert('Alerta',
        ' Gerente ainda não aprovou, verifique !!!'));
    end;
  end;
...
begin
  ...
  begin
    ...
    // Deleta o que já existe   //Evita duplicidade
    q_geral.Close;
    q_geral.sql.Clear;
    q_geral.sql.Text :=
...
begin
  ...
  begin
    ...
    begin
      ...
      AddToInitProc(swalAlert('Alerta',
        ' Campos obrigatórios não preenchidos:SPMC '));
  ...
  begin
    ...
    begin
      AddToInitProc(swalAlert('Alerta',
        ' Campos obrigatórios não preenchidos:GAP '));
    end
    else
    begin
...
begin
  ...
  begin
    result := false;
    campo := campo + '--Família do Veículo--';
  end;
  if combo_modelos.KeyValue = '-1' then
  begin
    result := false;
    campo := campo + '--Modelo do Veículo--';
  end;

  if combo_cores_externas.KeyValue = '-1' then
  begin
    result := false;
    campo := campo + '--Cor do Veículo--';
  end;
  ...
  begin
    ...
    AddToInitProc(swalAlert('Alerta', ' Campos obrigatórios não preenchidos: '
      + campo));
...
begin
  ...
  begin
    ...
    begin
      AddToInitProc(swalAlert('Alerta', 'Acesso:WC0009, não permitido!'));
...
begin
  inherited;
  // AddToInitProc(swalSuccess('Confirmação', 'Código '+combo_origem.Text));
end;
...
begin
  ...
  begin
    ...
    AddToInitProc(swalAlert('Informação',
  ...
  begin
    AddToInitProc(swalAlert('Informação',
      'Verificação concluída, não foram encontrados registros anteriores para este cliente.')
      );
  end;
...
begin
  ...
  ATag.Add(' placeholder= " Informe um fone válido. " ');
end;
...
begin
  ...
  begin
    validar := 'N';
    campos_obg := campos_obg + '-Valor do Veíc. -';
  end;
  ...
  begin
    ...
    if ck_aprovar_acessorios.Checked then
    begin
      AddToInitProc(swalSuccess('Confirmação', 'Salvo/Aprovado com sucesso!'));
    end
    else
    begin
      AddToInitProc(swalSuccess('Confirmação', 'Salvo com sucesso!'));
    end;
  ...
  begin
    ...
    AddToInitProc(swalAlert('Erro!',
      ' Campos Obrigatórios, precisam ser preenchidos:' + campos_obg));
  end;
...
begin
  inherited;
  // Aqui deve-se fazer o registro das CallBacks necessárias para o uso
  // 'Tabela'     => Nome do Callback no DataTable dentro do JS na linha ajax: ...
  // MontaTabela  => Nome da Procedure que será executada toda vez que o DataTable acionar o ajax
...
begin
  ...
  begin
    ...
    AddToInitProc(swalSuccess('Confirmação', ' Simulação Criada com Sucesso nº:'
      + lb_id.Caption));
  end
  else
  begin
    ...
    // verifica aprovações e desabilita campos
...
begin
  ...
  if copy(iwacao.Text, 1, 5) = 'SAIRF' then // Insere histório de financiamento
  begin
    if controller.get_acesso('WC0049') = 1 then
    begin
  ...
  if copy(iwacao.Text, 1, 5) = 'INCFI' then // Insere histório de financiamento
  begin
    inserir_fi_historico;
  ...
  begin
    ...
    q_geral.ExecSQL;
    AddToInitProc(swalSuccess('Confirmação', ' Financiamento: ' + ID +
  ...
  begin
    ...
    q_geral.ExecSQL;
    AddToInitProc(swalSuccess('Confirmação', ' Documento: ' + ID +
      ', excluído com sucesso !!!'));
  ...
  if copy(iwacao.Text, 1, 5) = 'ExcDo' then // exclue documento da simulação
  begin
    ID := copy(iwacao.Text, 6, length(iwacao.Text));
...
begin
  ...
  begin
    ...
    begin
      ...
      tabela := tabela + '<thead>';
      tabela := tabela + '<tr>';
      tabela := tabela + '<th>Analista de Crédito</th>';
      ...
      time_line_item := time_line_item + '';
      time_line_item := time_line_item +
        '<h3 class="timeline-header"><a href="#">Usuário:' +
  ...
  begin
    ...
    VValue := VValue +
      'src="https://cdn2.iconfinder.com/data/icons/32pxmania/misc_18.png"';
    VValue := VValue +
      'style="width: 20px; height: 25px"><span class="badge">Valores da Negociação</span></a>';
    VValue := VValue + v_info;
    VValue := VValue + '</h4>';
...
begin
  inherited;
  ATag.Add(' placeholder = ''Campo para observações de status''  ');
end;
...
begin
  ...
  begin
    ...
    // Alimenta as variáveis de parâ,etros:
