...
type
  TUsuariosOrigem = class(TFrmPadrao)
    ...
    lb_acao: TIWLabel;
    list_docs_selecao: TIWListbox;
    btn_add_doc: TIWButton;
    btn_del_doc: TIWButton;
    list_docs_selecionados: TIWListbox;
    ...
    procedure btn_add_docAsyncClick(Sender: TObject; EventParams: TStringList);
    procedure btn_del_docAsyncClick(Sender: TObject; EventParams: TStringList);
  private
...
procedure TUsuariosOrigem.btn_add_docAsyncClick(Sender: TObject;
  EventParams: TStringList);
begin
  inherited;
  if list_docs_selecao.ItemIndex.ToString <> '-1' then
  begin

    list_docs_selecionados.Items.Add(list_docs_selecao.SelectedValue);
    list_docs_selecao.Items.Delete(list_docs_selecao.ItemIndex);

  end;
end;

procedure TUsuariosOrigem.btn_del_docAsyncClick(Sender: TObject;
  EventParams: TStringList);
begin
  inherited;
  if list_docs_selecionados.ItemIndex.ToString <> '-1' then
  begin

    list_docs_selecao.Items.Add(list_docs_selecionados.SelectedValue);
    list_docs_selecionados.Items.Delete(list_docs_selecionados.ItemIndex);
...
begin
  ...
  begin
    ...
    q_geral.SQL.Clear;
    q_geral.SQL.Add
      (' insert into wc_origens(id,descricao,ativo,aprova_acessorios)values(seq_wc_geral.nextval, :descricao , :ativo , :aprova_acessorios )');
    ...
    begin
      q_geral.ParamByName('aprova_acessorios').AsString := 'S'
    end
    else
    begin
      q_geral.ParamByName('aprova_acessorios').AsString := 'N'
    end;

    q_geral.ExecSQL;


    //Faz cruzamento com documentos

    // Apaga existentes
       q_geral.Close;
       q_geral.SQL.Clear;
       q_geral.SQL.Add(' delete from wc_origens_doc where origem='+ quotedstr(edit_origem.Text));
       q_geral.ExecSQL;


     for i := 0 to list_docs_selecionados.Items.Count -1 do
       begin

        q_geral.Close;
        q_geral.SQL.Clear;
        q_geral.SQL.Add(' insert into  wc_origens_doc(id,origem,doc) values( seq_wc_geral.nextval, :origem , :doc)');
        q_geral.ParamByName('origem').AsString:= edit_origem.Text;
        q_geral.ParamByName('doc').AsString:= list_docs_selecionados.Items[i];
        q_geral.ExecSQL;

       end;


    AddToInitProc(swalSuccess('Confirmação', 'Inclusão Efetuada com sucesso!'));
    WebApplication.CallBackResponse.AddJavaScriptToExecute
...
begin
  ...
  begin
    ...
    q_geral.SQL.Clear;
    q_geral.SQL.Add
      ('select  descricao from wc_lista_documentos  a where ativo=''S'' ');
      q_geral.SQL.Add( '  and not exists( select 1 from WC_ORIGENS_DOC b where b.doc=a.descricao and origem= :origem )  ' );
      q_geral.SQL.Add(' order by descricao');
   q_geral.ParamByName('origem').AsString:=edit_origem.Text;

    q_geral.Open();

    q_geral.First;
    list_docs_selecao.Items.Clear;
    while not q_geral.Eof do
    begin

      list_docs_selecao.Items.Add(q_geral.FieldByName('descricao').AsString);

      q_geral.Next;

    end;

    list_docs_selecionados.Items.Clear;

    q_geral.Close;
    q_geral.SQL.Clear;
    q_geral.SQL.Add
      ('select doc from WC_ORIGENS_DOC t where origem = :origem order by doc');
   q_geral.ParamByName('origem').AsString:= edit_origem.Text;
    q_geral.Open();
    ...
    begin
      ...
      list_docs_selecionados.Items.Add(q_geral.FieldByName('doc').AsString);
...
begin
  ...
  begin
    ...
    // '      <li><a onclick=\"SetaAcao(''FAPAC'')\" > <i class=\"fa fa-thumbs-o-up\"></i> Aprovar Acessórios </a></li>           ' +
    // '      <li><a onclick=\"SetaAcao(''FDPAC'')\" > <i class=\"fa fa-thumbs-o-down\"></i> Desaprovar Acessórios </a></li>         ' +
    ...
    // '      <li><a onclick=\"SetaAcao(''FEXCL'')\" > <i class=\"fa  fa-trash\"></i> Excluir Ficha(Não Aprovada)</a></li>        ' +
    // '   </ul>                                                                                                                  ' +
    // ' </div>"                                                                                                                  ' ;
