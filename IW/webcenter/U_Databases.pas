...
begin
  ...
  begin
    ...
    AddToInitProc(swalAlert('Alerta', ' Campo usuário não pode ser vazio!'));
    erro := 'S';
  ...
  begin
    ...
    AddToInitProc(swalAlert('Alerta', ' Campo Senha não pode ser vazio!'));
    erro := 'S';
  end;

  if edit_servidor.Text = '' then
  begin

    AddToInitProc(swalAlert('Alerta', ' Campo servidor não pode ser vazio!'));
    erro := 'S';
  end;
  ...
  begin
    ...
    AddToInitProc(swalAlert('Alerta', ' Campo descricao não pode ser vazio!'));
    erro := 'S';
  end;
  ...
  begin
    ...
    q_datatable.SQL.Add(' select max(id)+1 as id from users_db ');
    q_datatable.Open();

    id := q_datatable.FieldByName('id').AsInteger;

    q_datatable.close;
    q_datatable.SQL.Clear;
    q_datatable.SQL.Add
      (' insert into users_db(id,usuario,senha,database,descricao) values(:id , :usuario , :senha , :database , :descricao ) ');
    q_datatable.ParamByName('id').AsInteger := id;
    ...
    WebApplication.CallBackResponse.AddJavaScriptToExecute
      ('$("#Databases").DataTable().ajax.reload();');
  end;
...
begin
  ...
  begin
    ...
    q_datatable.close;
    q_datatable.SQL.Clear;
    q_datatable.SQL.Add(' delete from users_db where id=' + id);
    q_datatable.ExecSQL;
    WebApplication.CallBackResponse.AddJavaScriptToExecute
      ('$("#Databases").DataTable().ajax.reload();');
    AddToInitProc(swalSuccess('Confirmação', ' Database ' + id +
...
begin
  ...
  q_datatable.SQL.Clear;
  q_datatable.SQL.Add('select id, USUARIO, descricao, database from users_db');
