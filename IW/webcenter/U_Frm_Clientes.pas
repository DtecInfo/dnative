...
type
  TFormClientes = class(TFrmPadrao)
    ...
    procedure BTN_IMPRIMIR_SIMAsyncClick(Sender: TObject;
      EventParams: TStringList);
    procedure BTN_IMPRIMIR_NAOAsyncClick(Sender: TObject;
      EventParams: TStringList);
  private
...
procedure TFormClientes.BTN_IMPRIMIR_NAOAsyncClick(Sender: TObject;
  EventParams: TStringList);
  var
  a : string;
begin
...
procedure TFormClientes.BTN_IMPRIMIR_SIMAsyncClick(Sender: TObject;
  EventParams: TStringList);
begin
  inherited;
    q_datatable.close;
    q_datatable.SQL.Clear;
    q_datatable.SQL.Add(' delete from clientes where cod_cliente like  ' + quotedstr('%'+ controller.xdelcod_cliente + '%'));
    q_datatable.ExecSQL;
    WebApplication.CallBackResponse.AddJavaScriptToExecute
      ('$("#clientes").DataTable().ajax.reload();');
    AddToInitProc(swalSuccess('Confirmação', ' Cliente: ' + controller.xdelcod_cliente +
...
//      ', excluído com sucesso !!!'));
  ...
  if copy(iwacao.Text, 1, 5) = 'AltCl' then
  begin
    id := copy(iwacao.Text, 6, Length(iwacao.Text));

    controller.xcod_cliente_alt:=  id;
    controller.AcaoMenu('FormCliCad');
