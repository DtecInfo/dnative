    ...
    controller.DM.banco.StartTransaction;
     ...
     begin
      //Itens Trib
        ...
        addtoinitproc(swalSuccess('Confirmação','Naturezas gravadas com sucesso!'));
      ...
      begin
        ...
        addtoinitproc(swalError('Erro!','Favor Selecionar as três naturezas!'));
...
begin
  ...
  begin
      ...
      try
        ...
        addtoinitproc(swalSuccess('Confirmação','Locações definidas com sucesso!'));
        WebApplication.CallBackResponse.AddJavaScriptToExecute('$("#TBItensXml").DataTable().ajax.reload();');
       ...
       begin
         ...
         addtoinitproc(swalError('Erro','Erro ao tentar sugestão:'+e.Message));
  ...
  begin
    addtoinitproc(swalAlert('Alerta!','Favor Selecionar um local Estoque válido'));
  end;
...
begin
 ...
 begin
      ...
      begin
        ...
        addtoinitproc(swalError('Erro','Dados da NF Não estão OK!-'+v_msg));

      end;
 end
 else
 addtoinitproc(swalAlert('Alerta!','Existem dados obrigatórios na Capa que não foram preenchidos. Verifique!'));
...
begin
  ...
  begin
   ...
   addtoinitproc(swalSuccess('Confirmação','Dados da NF estão OK!'));
...
begin
 ...
 if (ifnull(edit_qtde_itens.Text)='0') or (ifnull(edit_preco_unitario_item.Text)='0') then
    begin
     ...
     AddToInitProc(swalAlert('Alerta','Campos Qtde ou valor não podem ser nulo ou zero, verifique!'));

    end
    else
    begin

     //Mutiplica preço unitário pela quantidade

       if strtofloat(ifnull(edit_desconto.Text)) > 0 then  //Recalcula preço unitario ao mudar quantidade
          begin
...
//atualiza componentes
 ...
 //Verifica se faltou definir algum dado no XML
  ...
  begin
    AddToInitProc(swalAlert('Alerta','Verifique se esqueceu de definir CFOP dos Itens ou Locação'));
  end
  else
  begin
      ...
      // q_geral.SQL.Add( ' and cod_empresa= ' + inttostr(controller.xcod_empresa) );
       ...
       // q_geral.SQL.Add( ' and cod_empresa= ' + inttostr(controller.xcod_empresa) );
        ...
        WebApplication.CallBackResponse.AddJavaScriptToExecute
          ...
          addtoinitproc(swalSuccess('Informação','XML carregado com sucesso!'));
...
begin
  inherited;
ATag.Add('placeholder="Digite o código do produto"');
   ATag.Add(' style="text-align:right" ');
end;
...
begin
  ...
  result:= 'true';
    ...
    swalAlert('Alerta','Item não encontrado: '+ edit_cod_produto.Text);
  end;
...
begin
 ...
 begin
    ...
    msg := msg + 'Total dos Produtos não confere!';
 ...
 begin
    ...
    msg := msg + 'Total do IPI não confere!';
 ...
 begin
    ...
    msg := msg + 'Base do ICMS não confere!';
  ...
  while not q_temp_itens.Eof do
   begin
     ...
     //INSERIR LOCAÇÃO
         ...
         //verifica se já existe na locação
         q_geral.Close;
         q_geral.SQL.Clear;
         q_geral.SQL.Text:=
...
begin
  ...
  if copy(iwacao.Text, 1, 5) = 'EditI' then // Edita o Item Lançado na temporária
  begin
    ...
    controller.DM.banco.StartTransaction;
      ...
      try
       ...
       //Carregar itens da Nota fiscal
          ...
          for I := 0 to nodeinfNFe.ChildNodes.Count -1 do   //Varre os nós da nfe que contem os dados da nfe
             begin

                if nodeinfnfe.ChildNodes[i].NodeName='det' then   //verifica se esta no nó de produtos "DET"
                   begin
                     ...
                     Q_GERAL.ParamByName('cod_controle').AsInteger:=  v_cod_controle;
                      ...
                      if nodeDet.ChildNodes.FindNode('imposto').ChildNodes.FindNode('ICMS').ChildNodes[0].NodeName = 'ICMS00' then    //verifica se é tributado integralmente
                        begin
                        ...
                        if nodeDet.ChildNodes.FindNode('imposto').ChildNodes.FindNode('ICMS').ChildNodes[0].NodeName ='ICMS10' then    //verifica se é substituição tributária
                        begin
...
begin
 ...
 begin
   addtoinitproc(swalAlert('Alerta!','Não há o que lançar!'));
 end
 else
 begin
