...
begin
  ...
  begin
    ...
    frxReport1.Variables['Filtro'] := QuotedStr('Filtro Descrição: "'+Filtro.Text+'"');
  end;

  // ORDENACAO
  if Ordenacao.SelectedValue = 'Código' then
    Controller.DM.FDQQuery.SQL.Add('Order by IDProduto ');
  if Ordenacao.SelectedValue = 'Descrição' then
...
begin
  ...
  begin
    WebApplication.CallBackResponse.AddJavaScriptToExecute(swalConfirm('CONFIRMAÇÃO', 'Deseja excluir o produto nº '+Trim(Copy(IWAcao.Text,6,50)), 'warning', 'Excluir', 'Cancelar', 'BTNEXCLUIR'));
  end;


  // Clicou em Imprimir Relatórios
...
begin
  ...
  begin
    ...
    html := html + '<table id="GRID" class="table table-bordered table-striped table-hover"> '+SLineBreak+
                   ...
                   '            <th>Descrição do Produto</th> '+SLineBreak+
...
//  begin
//    WebApplication.CallBackResponse.AddJavaScriptToExecute(swalError('Atenção', 'Descrição do Produto precisa ter pelo menos 5 caracteres !'));
