...
begin
  inherited;
  ATag.Add('placeholder="Digite parte do nome da solicitação"');
end;
...
begin
  ...
  begin
    WebApplication.CallBackResponse.AddJavaScriptToExecute(swalConfirm('CONFIRMAÇÃO', 'Deseja excluir a solicitação nº '+Trim(Copy(IWAcao.Text,6,50)), 'warning', 'Excluir', 'Cancelar', 'BTNEXCLUIR'));
  end;
...
begin
  ...
  begin
    ...
    html := html + '<table id="GRID" class="table table-bordered table-striped table-hover"> '+SLineBreak+
                   ...
                   '            <th>Descrição da Solicitação</th> '+SLineBreak+
