...
begin
  ...
  //
  // Clique no Menu
  // Para usar o TWBSPagination temos que colocar o Menu aqui no onClick, pois não funciona se ficar no onAsyncClick
  //
...
begin
  ...
  if (AColumn = 5) and (ARow > 0) and (ACell.Text <> '') then
    ACell.Text := '<button type="button" class="btn btn-warning btn-xs" data-toggle="tooltip" data-placement="top" title="Editar Registro" onclick="SetaAcao(''Alt'','+ACell.Text+');"><span class="fa fa-pencil" aria-hidden="true"></span> </button>';
// SE NAO USAR SWEETALERT USAR COMO ABAIXO
//    ACell.Text := '<a href="#" onclick="return SubmitClick(''IWDBGRID'',''' + IntToStr(AColumn) + '_' + Acell.Text + ''', false);"> <button type="button" class="btn btn-primary btn-xs" title="Editar Registro"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> </button> </a>';
  ...
  if (AColumn = 6) and (ARow > 0) and (ACell.Text <> '') then
    ACell.Text := '<button type="button" class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Excluir Registro" onclick="SetaAcao(''Exc'','+ACell.Text+');"><span class="fa fa-trash" aria-hidden="true"></span> </button>';
...
//    ACell.Text := '<a href="#" onclick="return SubmitClickConfirm(''IWDBGRID'',''' + IntToStr(AColumn) + '_' + Acell.Text + ''', true, ''Confirma a Exclusão do Registro ?'');"> <button type="button" class="btn btn-danger btn-xs" title="Excluir Registro"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span> </button> </a>';
  ...
  if (AColumn = 7) and (ARow > 0) and (ACell.Text <> '') then
    ACell.Text := '<button type="button" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="top" title="Resetar Senha" onclick="SetaAcao(''Senha'','+ACell.Text+');"><span class="fa fa-key" aria-hidden="true"></span> </button>';

// SE NAO USAR SWEETALERT USAR COMO ABAIXO
//    ACell.Text := '<a href="#" onclick="return SubmitClickConfirm(''IWDBGRID'',''' + IntToStr(AColumn) + '_' + Acell.Text + ''', true, ''Confirma o Resete da Senha ?'');"> <button type="button" class="btn btn-danger btn-xs" title="Excluir Registro"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span> </button> </a>';
end;
