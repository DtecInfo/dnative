...
begin
 ...
 r_romaneio.Variables['total_itens']   := quotedstr(q_imp_compras_itens.RecordCount.ToString);
...
//  q_datatable.SQL.Add(' select count(*) as total  ');
  q_datatable.sql.add('SELECT count(*) as total FROM COMPRAS C, CLIENTES CLI');
 ...
 // q_datatable.SQL.Add(' where 1=1  ') ;
  ...
  q_datatable.SQL.Add('      OR  c.numero_nota like ''%' +
    ...
    q_datatable.SQL.Add('      OR  cli.nome like ''%' +
    stringreplace(UpperCase(aParams.Values['search[value]']), ' ', '%',
    [rfReplaceAll]) + '%''  ');
