...
begin
  ...
  try
    ...
    AddToInitProc(swalSuccess('CONFIRMAÇÃO', 'Encerramento do Chamado realizado com sucesso !'));

    MontaChamadosAbertos;

  except
    AddToInitProc(swalAlert('ATENÇÃO','Ocorreu um erro no Encerramento do Chamado, tente novamente !'));
  end;
...
begin
  ...
  try
    ...
    if not Encerrar.Checked then
      AddToInitProc(swalSuccess('CONFIRMAÇÃO', 'Atendimento realizado com sucesso !'))
    else
      AddToInitProc(swalSuccess('CONFIRMAÇÃO', 'Atendimento e Encerramento realizado com sucesso !'));
...
begin
  ...
  try
    ...
    AddToInitProc(swalSuccess('CONFIRMAÇÃO', 'Reabertura do Chamado realizado com sucesso !'));

    MontaChamadosAbertos;

  except
    AddToInitProc(swalAlert('ATENÇÃO','Ocorreu um erro na Reabertura do Chamado, tente novamente !'));
  end;
...
begin
  ...
  begin
    ...
    html := html + '                                        <dt>Nº Chamado:</dt> ' + SLineBreak;
    ...
    html := html + '                                        <br /> ' + SLineBreak;
    html := html + '                                        <dt>Solicitação:</dt> ' + SLineBreak;
  ...
  begin
    WebApplication.CallBackResponse.AddJavaScriptToExecute(swalConfirm('CONFIRMAÇÃO', 'Tem certeza que deseja Encerrar o Chamado nº '+Trim(Copy(IWAcao.Text,6,50)), 'warning', 'Encerrar Chamado', 'Cancelar', 'BTNENCERRAR'));
  end;
  ...
  begin
    WebApplication.CallBackResponse.AddJavaScriptToExecute(swalConfirm('CONFIRMAÇÃO', 'Tem certeza que deseja Reabrir o Chamado nº '+Trim(Copy(IWAcao.Text,6,50)), 'warning', 'Reabrir Chamado', 'Cancelar', 'BTNREABRIR'));
  end;
end;
...
begin
  ...
  html := html + '                    <span data-toggle="tooltip" title="Clique para visualizar" class="badge bg-blue">'+Controller.iif(Abertos > 0, Abertos.ToString+' Novos Chamados','Não há Novos Chamados')+'</span> ' + SLineBreak;
  html := html + '                </a> ' + SLineBreak;
  html := html + '                <a href="javascript: SetaAcao(''Fecha'', '''');"> ' + SLineBreak;
  html := html + '                    <span data-toggle="tooltip" title="Clique para visualizar" class="badge bg-red">'+Controller.iif(Fechados > 0, Fechados.ToString+' Chamados Fechados','Não há Chamados Fechados')+'</span> ' + SLineBreak;
  html := html + '                </a> ' + SLineBreak;
  html := html + '            </div> ' + SLineBreak;
  html := html + '        </div> ' + SLineBreak;
  html := html + '        <div class="box-body"> ' + SLineBreak;
  ...
  begin
    ...
    html := html + '                                </span> ' + SLineBreak;
    html := html + '                                <span data-toggle="tooltip" title="Nome do Módulo do Sistema"> ' + SLineBreak;
    ...
    html := html + '                                </span> ' + SLineBreak;
    html := html + '                                <span data-toggle="tooltip" title="Nome do Usuário"> ' + SLineBreak;
...
begin
  ...
  html := html + '        <div class="box-header with-border"> ' + SLineBreak;
  html := html + '            <h3 class="box-title"><i class="fa fa-commenting-o"></i> Listagem dos Chamados já Fechados</h3> ' + SLineBreak;
  html := html + '            <div class="box-tools pull-right"> ' + SLineBreak;
  html := html + '                <a href="javascript: SetaAcao(''Abert'', '''');"> ' + SLineBreak;
  html := html + '                    <span data-toggle="tooltip" title="Clique para visualizar" class="badge bg-blue">'+Controller.iif(Abertos > 0, Abertos.ToString+' Novos Chamados','Não há Novos Chamados')+'</span> ' + SLineBreak;
  html := html + '                </a> ' + SLineBreak;
  ...
  begin
    ...
    html := html + '                                        <dt>Nome do Atendente:</dt> ' + SLineBreak;
    html := html + '                                        <dd>'+Controller.DM.FDQQuery.FieldByName('NomeFuncionario').AsString+'</dd> ' + SLineBreak;
    html := html + '                                        <dt>Data / Hora Solução:</dt> ' + SLineBreak;
